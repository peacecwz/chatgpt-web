...
const Item: FC<ItemProps> = (props) => {
    const { icon, item, type, href, selected, onClick, onDelete, onRename } =
        props
...
const Sidebar: FC<{
    ...
    changeCurrent: (item: Conversation | null) => void
    ...
    return (
        <div className="bg-gray-900 w-72 p-2 h-full flex flex-col justify-between">
            <button className="w-full flex items-center px-2 space-x-2 py-3 text-gray-100 border border-gray-700 hover:bg-gray-800">
                <AiOutlinePlus />
                <span>New Chat</span>
            </button>
                ...
                {conversations.map((item, index) => (
                    <Item
                        ...
                        onDelete={(conversation) =>
                            fetch(
                                `/api/conversations/delete?id=${conversation.id}`,
                                {
                                    method: 'DELETE',
                                    headers: {
...
const ChatScreen: FC<{
    ...
    const handleKeyDown = async (
        ...
        if (event.key === 'Enter') {
            ...
            // TODO: Fetching conversation logic will be changed
        ...
        <div id="chat-screen" className="w-full flex flex-col h-screen">
                    ...
                    <ul className="mt-4">
                        {current.messages.map((message, index) => (
                            <li
                                ...
                                className={`flex py-6 space-x-2 py-2 px-4 border-b border-black/10 dark:border-gray-900/50 group ${
                                    message.role === 'system'
                            ...
                            >
                                <div className="flex px-96 items-start space-x-4">
                                    ...
                                    <div className="text-gray-800 text-gray-800 dark:text-white font-medium self-center space-y-4 max-w-2xl">
                                        <ReactMarkdown
                                                ...
                                                code({
                                                    ...
                                                    return !inline && match ? (
                                                        <div className="space-y-0 bg-black">
                                                            <div className="flex justify-between bg-gray-800 font-light text-white text-xs p-2">
                                                                <span>
                                                                    {match[1]}
                                                                </span>
                                                                <button
            ...
            <div className="flex sticky bottom-0 py-4 px-6 justify-center">
                <div className="relative w-full max-w-2xl">
                    ...
                    ></textarea>
                    <button className="absolute right-1 top-0.5 bottom-0.5 px-2 text-gray-400 hover:text-gray-600 focus:outline-none focus:text-gray-600">
                        <TbSend size={24} />
                    </button>
                </div>
            </div>
